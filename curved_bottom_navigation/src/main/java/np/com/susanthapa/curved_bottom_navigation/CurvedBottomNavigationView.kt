...
class CurvedBottomNavigationView @JvmOverloads constructor(
    ...
    private fun initializeMenuImages() {
        menuIcons = Array(cbnMenuItems.size) {
            val drawable = ResourcesCompat.getDrawable(resources, cbnMenuItems[it].imageResId, context.theme)!!
            DrawableCompat.setTint(drawable, if (it == selectedIndex) selectedColor else unSelectedColor)
            drawable
        }
    }
    ...
    private fun initializeMenuAVDs() {
        ...
        menuAVDs = Array(cbnMenuItems.size) {
            val avd = AnimatedVectorDrawableCompat.create(context, cbnMenuItems[it].imageResId)!!
...
//         pop to the navigation graph's start  destination
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // رسم تصویر در FAB
        menuIcons[selectedIndex].setBounds(
            (centerX - menuIcons[selectedIndex].intrinsicWidth / 2).toInt(),
            (curCenterY - menuIcons[selectedIndex].intrinsicHeight / 2).toInt(),
            (centerX + menuIcons[selectedIndex].intrinsicWidth / 2).toInt(),
            (curCenterY + menuIcons[selectedIndex].intrinsicHeight / 2).toInt()
        )
        menuIcons[selectedIndex].draw(canvas)

        // رسم مسیر نویگیشن
        canvas.drawPath(path, navPaint)

        // رسم تصاویر برای آیتم‌های پایین
        for (i in bottomNavItemViews.indices) {
            val itemCenterX = menuCellWidth * i + (menuCellWidth / 2f)

            menuIcons[i].setBounds(
                (itemCenterX - menuIcons[i].intrinsicWidth / 2).toInt(),
                (layoutHeight - menuIcons[i].intrinsicHeight - 20).toInt(),
                (itemCenterX + menuIcons[i].intrinsicWidth / 2).toInt(),
                (layoutHeight - 20).toInt()
            )
            menuIcons[i].draw(canvas)
        }
    }
